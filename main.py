...
def main(transactions, min_support=4, min_length=3, file_location='output.txt'):
    ...
    filtered_transactions, filtered_counts = pp.preprocess(transactions, min_support)
    ...
    for item in filtered_counts:
        ...
        # pass is min_length - 1 as the min_length_of_prefix because the
        # total length of a pattern will be prefix + item, or prefix + 1
        prefix_paths = tree.prefix_paths(item, min_length - 1) 
        conditional_fptree_elements = fp_helper.conditional_fptree_elements(prefix_paths, min_support)
        print(f'{item}: {conditional_fptree_elements}')
        frequent_patterns = fp_helper.frequent_patterns(item, conditional_fptree_elements)
